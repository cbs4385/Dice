...
- **Undrafted design forks** — the default information depth (bag counts vs counts + odds) is a **decision this plan does not make**. Do not pick one silently; implement against whichever the supervisor specifies, and escalate first if unspecified. (The other named fork, draft model, was resolved 2026-07-02: snake is the confirmed choice, not just a provisional default — see `docs/progress.md`. `IDraftOrderStrategy` stays swappable as good architecture, not because the choice is still open.)
